...
# 카카오의 5초 응답 제한(콜백 미승인 상태) 안에 답을 마치기 위해
# 더 빠른 모델을 사용합니다. 콜백이 승인되면 sonnet으로 다시 바꿔도 됩니다.
...
- 답변은 카카오톡 채팅창에서 읽기 좋도록 최대한 간결하게, 1~2문장으로만 작성하세요.
  (지금은 5초 안에 응답해야 하는 제약이 있어 짧은 답변이 필수입니다.)
...
async def ask_claude(question: str, law_context: str | None = None) -> str:
    ...
    try:
        # 5초 제한 안에 끝내야 하므로 타임아웃을 짧게 잡습니다.
...
async def kakao_skill(request: Request, background_tasks: BackgroundTasks):
    ...
    # 콜백이 지원되지 않는 환경(카카오 5초 제한 안에 답해야 함)에서는
    # 법령 조회(law.go.kr 왕복 통신)를 생략해 시간을 절약합니다.
    # 콜백이 승인되면 process_and_send_callback처럼 법령 조회를 다시 포함하면 됩니다.
    answer = await ask_claude(question, law_context=None)
